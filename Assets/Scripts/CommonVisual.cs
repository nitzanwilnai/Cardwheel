    ...
    public static class CommonVisual
    {
            ...
            for (int jkrIdx = 0; jkrIdx < runData.JokerCount; jkrIdx++)
            {
                ...
                JokerGUIs[jkrIdx]
                    .Button.onClick.AddListener(() =>
                        Game.Instance.ShowJokerInfoPopupInGame(localJokerIdx)
                    );
            }
        }
        ...
        public static void ShowJokers(RunData runData, Balance balance, Transform jokerParent)
        {
            ...
            for (int jkrIdx = 0; jkrIdx < runData.JokerCount; jkrIdx++)
            {
                ...
                JokerGUIs[jkrIdx]
                    .Button.onClick.AddListener(() =>
                        Game.Instance.ShowJokerInfoPopup(localJokerIdx)
                    );
            }
        }
        ...
        public static void ShowJokersCommon(RunData runData, Balance balance, Transform jokerParent)
        {
            ...
            for (int jkrIdx = 0; jkrIdx < runData.JokerCount; jkrIdx++)
            {
                ...
                JokerGUIs[jkrIdx].CardImage.sprite = AssetManager.Instance.LoadJokerSprite(
        ...
        static void ShowJokerDescriptionWithIndex(
            RunData runData,
            Balance balance,
            GameObject go,
            int jokerType,
        ...
        )
        {
            if (balance.JokerBalance.ChipsIncreasePerSpin[jokerType] > 0)
            {
                ...
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current " + chipIncrease.ToString("N0") + ")";
            }

            if (
                balance.JokerBalance.MultIncreasePerUnusedSpin[jokerType] != 0
            ...
            )
            {
                ...
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current " + multiplierAdd.ToString("N0") + "x)";
            }
            ...
            {
                double multiplierAdd =
                    jokerIdx > -1
                        ? runData.JokerMultiplierAdd[jokerIdx]
                        : balance.JokerBalance.SubtractMultiplierAddPerRound[jokerType].x;
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current " + multiplierAdd.ToString("N0") + "x)";
            }
            ...
            if (balance.JokerBalance.MultAddIncreasePerBall[jokerType] > 0)
            {
                double multiplierAdd = jokerIdx > -1 ? runData.JokerMultiplierAdd[jokerIdx] : 0.0f;
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current " + multiplierAdd.ToString("N1") + "x)";
            }

            if (balance.JokerBalance.MultMultIncreasePerBall[jokerType] > 0)
            {
                double multiplierMult =
                    jokerIdx > -1 ? runData.JokerMultiplierMult[jokerIdx] : 0.0f;
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current x" + multiplierMult.ToString("N2") + ")";
            }
            ...
            if (balance.JokerBalance.BallIncMultRemoveSlotMod[jokerType] > 0)
            {
                ...
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current " + multIncrease + ")";
            }
            if (balance.JokerBalance.IncreaseSellValueEveryRound[jokerType] > 0)
            {
                int sellValue =
                    jokerIdx > -1
                        ? runData.JokerSellValues[jokerIdx]
                        : balance.JokerBalance.Cost[jokerType] / 2;
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current ◇" + sellValue + ")";
            }
        ...
        public static void ShowJokerDescriptionCommon(
            RunData runData,
            Balance balance,
            GameObject go,
            int jokerType,
        ...
        )
        {
            ...
            if (balance.JokerBalance.PerJokerMultiplierAdd[jokerType] > 0)
            {
                ...
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current +" + ((runData.JokerCount + 1) * chips) + "x)";
            }
            if (balance.JokerBalance.MultiplierMultForSpecialBall[jokerType] > 0.0f)
            {
                ...
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    ...
                    + (
                        numSpecialBalls
                        * balance.JokerBalance.MultiplierMultForSpecialBall[jokerType]
                    )
                    + ")";
            }
            ...
            if (balance.JokerBalance.MultiplierMultEveryShopReroll[jokerType] > 0.0f)
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    ...
                    + (
                        ...
                        * balance.JokerBalance.MultiplierMultEveryShopReroll[jokerType]
                    )
                    + ")";
            ...
            if (balance.JokerBalance.ChipsPerDollar[jokerType] > 0)
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current "
                    + (balance.JokerBalance.ChipsPerDollar[jokerType] * runData.Money).ToString(
                        "N0"
                    )
                    + ")";
            ...
            {
                int chipsAdd =
                    Logic.GetNumNonModedSlots(runData, balance)
                    * balance.JokerBalance.ChipsAddForEveryNonSlotMod[jokerType];
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current " + chipsAdd.ToString("N0") + ")";
            }

            if (balance.JokerBalance.MultiplierAddForEverySlotMod[jokerType] > 0.0f)
            {
                float multiplierAdd =
                    Logic.GetNumModedSlots(runData, balance)
                    * balance.JokerBalance.MultiplierAddForEverySlotMod[jokerType];
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "(Current " + multiplierAdd.ToString("N0") + "x)";
            }
            ...
            if (balance.JokerBalance.MultiplierAddForLeastPlayedColor[jokerType] > 0.0f)
            {
                ...
                go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                    "<color=#"
                    + ColorUtility.ToHtmlStringRGBA(balance.SlotColors[(int)slotType])
                    + ">"
                    + slotType.ToString()
                    + "</color>";
            }
            ...
            if (balance.JokerBalance.AddAllSellValueToMult[jokerType])
            {
                ...
                for (int jkrIdx = 0; jkrIdx < runData.JokerCount; jkrIdx++)
                {
                    ...
                    go.GetComponent<GUIRef>().GetTextGUI("Current").text =
                        "(Current +x" + totalSellValue.ToString("N0") + ")";
                }
            }
        }
        ...
        public static void ShowBallsAndSpinWheel(
            RunData runData,
            Balance balance,
            CardsBallsSpinWheelGUI cardsBallsSpinWheelGUI,
            SLOT_TYPE[] slotType
        )
        {
            cardsBallsSpinWheelGUI.SortingPopup.SetActive(false);
            ShowBalls(runData, balance, cardsBallsSpinWheelGUI);
            CommonSlotsVisual.ShowSpinWheelUI(
                runData,
                balance,
                cardsBallsSpinWheelGUI.ScoringSlots,
                slotType
            );
        }

        public static void ShowJokersBallsAndSpinWheel(
            RunData runData,
            Balance balance,
            CardsBallsSpinWheelGUI cardsBallsSpinWheelGUI,
            SLOT_TYPE[] slotType
        ...
        )
        {
            for (int ballIdx = 0; ballIdx < balance.MaxBalls; ballIdx++)
            {
                ...
                cardsBallsSpinWheelGUI.Balls[ballIdx].sprite = AssetManager.Instance.LoadBallSprite(
                    balance.BallBalance.BallSprite[ballType]
                );
        ...
        public static void InitCardsBallsSpinWheelGUI(
            Balance balance,
            GameObject go,
            ref CardsBallsSpinWheelGUI cardsBallsSpinWheelGUI
        )
        {
            ...
            cardsBallsSpinWheelGUI.BallsButtonData.Button.onClick.AddListener(
                Game.Instance.GoToBallScreen
            );
            cardsBallsSpinWheelGUI.SpinwheelButtonData.Button.onClick.AddListener(
                Game.Instance.GoToChipsInfo
            );
            CommonButtonVisual.AddSelectedBorder(cardsBallsSpinWheelGUI.BallsButtonData);

            SpinWheelRef spinWheelRef = guiRef
                .GetGameObject("SpinWheel")
                .GetComponent<SpinWheelRef>();
            ...
            for (int i = 0; i < spinWheelRef.SlotGO.Length; i++)
                cardsBallsSpinWheelGUI.ScoringSlots[i] = spinWheelRef
                    .SlotGO[i]
            ...
            for (int i = 0; i < title.Length; i++)
            {
                colorTitle +=
                    ...
                    + ColorUtility.ToHtmlStringRGBA(
                        balance.SlotColors[i % balance.SlotColors.Length]
                    )
                    + ">";
                colorTitle += title[i];
                colorTitle += "</color>";
        ...
        {
            return (smallRound < 2 ? "Round " : "Boss ")
                + (bigRound + 1).ToString()
                + " - "
                + (smallRound + 1).ToString();
        }
        ...
        public static string GetBossDescription(RunData runData, Balance balance, string title)
        {
            ...
            if (
                ...
                == BOSS_EFFECT.LOSE_MONEY_EVERY_BALL_MOST_COMMON_COLOR
            )
                bossText += " (" + Logic.GetMostPlayedSlotType(runData).ToString() + ")";
        ...
        public static void ShowRoundInGameInfo(
            RunData runData,
            Balance balance,
            int bigRound,
            int i,
        ...
        public static void ShowRoundShopInfo(
            RunData runData,
            Balance balance,
            int bigRound,
            int i,
        ...
        private static string getSkipDescriptionCurrent(
            RunData runData,
            Balance balance,
            int skipType
        )
        {
            ...
            if (balance.SkipBalance.MoneyForSpinsUsed[skipType] > 0)
                extra =
                    "\n(Current ◇"
                    + (
                        runData.SpinsUsed * balance.SkipBalance.MoneyForSpinsUsed[skipType]
                    ).ToString("N0")
                    + ")";
            if (balance.SkipBalance.MoneyForSpinsUnused[skipType] > 0)
                extra =
                    "\n(Current ◇"
                    + (
                        runData.SpinsUnused * balance.SkipBalance.MoneyForSpinsUnused[skipType]
                    ).ToString("N0")
                    + ")";
            return extra;
        }
        ...
        )
        {
            ...
            CardPackCommonVisual.ShowCards(
                runData,
                balance,
                cardPackCardGUIs,
                descriptionGOs,
            ...
            CardPackCommonVisual.ShowRerollButton(
                runData,
                balance,
                rerollButtonData.Button,
                rerollCostText
            );
        }
        ...
        public static void InitSpinWheel(
            GUIRef guiRef,
            ref SpinCircle m_spinCircle,
            ref ScoringSlot[] m_scoringSlots
        )
        {
            SpinWheelRef spinWheelRef = guiRef
                .GetGameObject("SpinWheel")
        ...
        public static void ShowAbandonedCardPackPopups(
            RunData runData,
            Balance balance,
            int abandonCount
        )
        {
            if (runData.CardPackAbandonTotal > abandonCount)
            {
                for (int jkrIdx = 0; jkrIdx < runData.JokerCount; jkrIdx++)
                {
                    int jokerType = runData.JokerTypes[jkrIdx];
                    {
                        ...
                        {
                            JokerGUIs[jkrIdx].JokerChips.SetActive(false);
                            JokerGUIs[jkrIdx].JokerChipsText.text =
                                "+" + balance.JokerBalance.ChipsAddForCardPackAbandon[jokerType];
                            JokerGUIs[jkrIdx].JokerChips.SetActive(true);
                        }
                        if (balance.JokerBalance.MultiplierMultForCardPackAbandon[jokerType] > 0)
                        {
                            JokerGUIs[jkrIdx].JokerMult.SetActive(false);
                            JokerGUIs[jkrIdx].JokerMultText.text =
                                "+"
                                + balance.JokerBalance.MultiplierMultForCardPackAbandon[jokerType]
                                + "x";
                            JokerGUIs[jkrIdx].JokerMult.SetActive(true);
                        }
                    }
                }
            }
        ...
        public static void ShowShopRerollPopups(RunData runData, Balance balance)
        {
            for (int jkrIdx = 0; jkrIdx < runData.JokerCount; jkrIdx++)
            {
                int jokerType = runData.JokerTypes[jkrIdx];
                {
                    if (balance.JokerBalance.MultiplierMultEveryShopReroll[jokerType] > 0)
                    {
                        JokerGUIs[jkrIdx].JokerMult.SetActive(false);
                        JokerGUIs[jkrIdx].JokerMultText.text =
                            "+"
        ...
        public static void ShowCardPackRerollPopups(
            RunData runData,
            Balance balance,
            int rerollCount
        )
        {
            if (runData.CardPackRerollCount > rerollCount)
            {
                for (int jkrIdx = 0; jkrIdx < runData.JokerCount; jkrIdx++)
                {
                    int jokerType = runData.JokerTypes[jkrIdx];
                    {
                        if (balance.JokerBalance.MultiplierMultEveryCardPackReroll[jokerType] > 0)
                        {
                            JokerGUIs[jkrIdx].JokerMult.SetActive(false);
                            JokerGUIs[jkrIdx].JokerMultText.text =
                                "+"
